...
import { RenderEvakusiSite } from '../Evakuasi/Component'

class CommentEmergencyScreen extends React.Component {
  state = {
    messages: [],
    loading: true,
    action: false
  ...
  componentDidUpdate() {
    ...
    if (this.state.action === true) {
      FireStoreListener.updatecommentEmergency(id, this.state.messages)
  ...
  check = async () => {
    const FireStoreListener = new Firestore()
    const { id } = this.props.navigation.state.params
    await FireStoreListener.onSnapshotcommentEmergency(id, (res) => {
      if (!res) {
        this.setState({ loading: false })
        FireStoreListener.updatecommentEmergency(id, [])
      } else {
        this.setState({ loading: false })
  ...
  render() {
    ...
    const { title, coordinate, address } = navigation.state.params
    return (
      <Container loading={this.loading} style={{ marginTop: 15 }}>
        <RenderEvakusiSite emergency={true} coordinate={coordinate} evacuationSite={title} address={address} />
        <Container>
...
CommentEmergencyScreen.navigationOptions = () => ({
  title: 'Komentar'
})
...
export default connect(mapStateToProps, mapDispatchToProps)(CommentEmergencyScreen)
